...
const crearCurso = (nuevoCurso) => {
	...
	if (camposObligatorios.length > 0) {
		...
		resultado['msg'] = 'Debe completar los campos obligatorios';
		return resultado;
	}
	...
	if (aspirante) {
		...
		if (aspiranteCurso) {
			...
			resultado['msg'] = `El aspirante con documento ${nuevoAspirante.doc} se ha 
				matriculado exitosamente en el curso con codigo ${id_curso}`;
			return resultado;
		}
	} else {
		aspiranteCurso = {
		...
		resultado['msg'] = `El aspirante con documento ${aspiranteCurso.doc_aspirante} se ha 
			matriculado exitosamente en el curso con codigo ${aspiranteCurso.id_curso}`;
		return resultado;
	}
};
...
const actualizarEstadoCurso = (idCurso, nuevoEstado) => {
	...
	if (cursoIndice < 0){
		...
		resultado['boton'] = 'actualizar';
		resultado['id_curso'] = idCurso;
		resultado['msg'] = `Curso con código no existente ${idCurso}. 
			Actualización no realizada satisfactoriamente`;
		return resultado;
	} else {
